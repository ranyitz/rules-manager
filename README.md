...
Presets allow you to bundle multiple rules & mcps into a single configuration that can be shared across projects. Presets themselves can also leverage glob patterns to define their rules, making it easy to manage a collection of rules within the preset package.
...
- **rules**: Object containing rule configurations. Each key-value pair defines a rule or a set of rules:

  - **Using a direct path (string):**
    - `rule-name`: A unique identifier for the rule. Can include a directory path (e.g., `my-group/my-rule`) to install the rule into a specific subdirectory.
    - `source-location`: Location of the rule file (path within an npm package or local path, e.g., `@my-team/ai-tools/rules/team-standards.mdc` or `./local/rule.mdc`).
    ```json
    "rules": {
      "team-rules/team-standards": "@my-team/ai-tools/rules/team-standards.mdc",
      "my-local-rule": "./rules/custom.mdc"
    }
    ```
  - **Using a glob pattern (object):**
    - `"arbitrary-key-name"`: A descriptive key for this glob definition. This key itself is not used as a rule name unless a file happens to match it.
    - `{ "glob": "pattern" }`: An object where the `glob` property is a glob pattern (e.g., `rules-directory/*.mdc`, `components/**/*.mdc`).
      - `aicm` will find all `.mdc` files matching the pattern relative to the configuration file (`aicm.json` or preset file).
      - The filename (without the `.mdc` extension) will be used as the `rule-name` for each discovered file.
      - This is useful for automatically picking up all rules from a directory.
    ```json
    "rules": {
      "component-rules": { "glob": "src/components/**/*.mdc" },
      "general-guidelines": { "glob": "docs/guidelines/*.mdc" }
    }
    ```
    Explicitly defined rules (using direct paths) take precedence if their `rule-name` conflicts with a name generated by a glob pattern within the same configuration file.
