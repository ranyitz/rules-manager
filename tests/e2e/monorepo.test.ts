...
describe("aicm monorepo support", () => {
  test("should discover and install rules from multiple packages", async () => {
    await setupFromFixture("monorepo-npm-basic");

    const { code, stdout } = await runCommand(
      "install --monorepo --ci --verbose",
    );
  ...
  test("should show error when no packages found in monorepo", async () => {
    await setupFromFixture("monorepo-no-packages");

    const { code, stderr } = await runCommand("install --monorepo --ci");

    expect(code).toBe(1);
    expect(stderr).toContain(
      "No packages with aicm configurations found in monorepo",
    );
  });

  test("should install normally when monorepo flag is used on single package", async () => {
  ...
  test("should handle partial configurations (some packages with configs, some without)", async () => {
    await setupFromFixture("monorepo-partial-configs");
  ...
  test("should discover and install rules from deeply nested npm workspace structure", async () => {
    await setupFromFixture("monorepo-npm-nested");
  ...
  test("should discover and install rules from Bazel monorepo", async () => {
    await setupFromFixture("monorepo-bazel-basic");
  ...
  test("should discover and install rules from mixed npm + Bazel monorepo", async () => {
    await setupFromFixture("monorepo-mixed");
  ...
  test("should handle error scenarios with invalid configurations", async () => {
    await setupFromFixture("monorepo-error-scenarios");
  ...
  test("should work quietly by default without verbose flag", async () => {
    ...
    const { code, stdout } = await runCommand("install --monorepo --ci");
