...
test("handle errors with missing rule files", async () => {
  await setupFromFixture("presets-missing-rules");

  // In v2, missing rule files in presets don't cause errors since it uses rulesDir scanning
  // Instead, let's test that the installation succeeds even with an empty preset rulesDir
  const { stdout, code } = await runCommand("install --ci");
